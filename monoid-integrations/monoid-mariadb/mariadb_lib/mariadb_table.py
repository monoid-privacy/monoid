...
def type_to_jsonschema(mariadb_type: str) -> Optional[str]:
    int_types = [
      "int",
      "integer",
      "mediumint",
    ...
    if mariadb_type in int_types:
        return "integer"
    elif mariadb_type in number_types:
        return "number"
    elif (mariadb_type in string_types) or (mariadb_type in time_types):
        return "string"
    return None
...
class MariaDBTableDataStore(DBDataStore):
    def __init__(self, table: str, db_name: str, conn: mariadb.connection):
        self.conn = conn
        self.table = table
